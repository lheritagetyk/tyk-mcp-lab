...
display_help() {
    ...
    echo
    echo "Brings up the slim Tyk MCP lab deployment."
    echo
    echo "Deployments (in addition to the auto-bootstrapped 'tyk' base):"
    echo "  mcp-gateway   MCP mock server + MCP Inspector"
    echo "  otel-jaeger   Jaeger all-in-one for OpenTelemetry traces"
    echo
    echo "Options:"
    ...
    echo
    echo "Examples:"
    echo "  ./up.sh                              # Bring up default Tyk base only"
    echo "  ./up.sh mcp-gateway otel-jaeger      # Full lab stack"
    echo "  ./up.sh mcp-gateway otel-jaeger --skip-hostname-check"
}
...
# Enable OpenTelemetry export when otel deployments are requested
if [[ "$*" == *otel* ]]; then
...
printf "\nTyk MCP lab initialisation process completed"
printf "\n--------------------------------------------\n\n"
